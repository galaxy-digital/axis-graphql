...
// OutstandingSAXIS resolves the amount of outstanding sAXIS tokens
// minted for this account.
func (del Delegation) OutstandingSAXIS() (hexutil.Big, error) {
	val, err := repository.R().DelegationOutstandingSAXIS(&del.Address, del.Delegation.ToStakerId)
	if err != nil {
		return hexutil.Big{}, err
	}
	return *val, nil
