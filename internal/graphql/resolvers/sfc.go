...
// CurrentEpoch resolves the id of the current epoch of the AXIS blockchain.
func (rs *rootResolver) CurrentEpoch() (hexutil.Uint64, error) {
	return repository.R().CurrentEpoch()
}

// LastStakerId resolves the last staker id in AXIS blockchain.
...
// StakersNum resolves the number of stakers in AXIS blockchain.
