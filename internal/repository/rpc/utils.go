...
func (axis *AxisBridge) GasPrice() (hexutil.Big, error) {
	// keep track of the operation
	axis.log.Debugf("checking current gas price")

	// call for data
	var price hexutil.Big
	err := axis.rpc.Call(&price, "axis_gasPrice")
	if err != nil {
		axis.log.Error("current gas price could not be obtained")
		return price, err
	}
...
func (axis *AxisBridge) GasEstimate(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	axis.log.Debugf("calling for gas amount estimation")

	var val hexutil.Uint64
	err := axis.rpc.Call(&val, "axis_estimateGas", trx)
	if err != nil {
		// missing required argument? incompatibility between old and new RPC API
		if strings.Contains(err.Error(), "missing value") {
			return axis.GasEstimateWithBlock(trx)
		}

		// return error
		axis.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
	}
...
func (axis *AxisBridge) GasEstimateWithBlock(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	// keep track of the operation
	axis.log.Debugf("calling for gas amount estimation with block details")

	var val hexutil.Uint64
	err := axis.rpc.Call(&val, "axis_estimateGas", trx, BlockTypeLatest)
	if err != nil {
		// return error
		axis.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
