...
import (
	"axis-graphql/internal/repository/db"
...
func (p *proxy) DelegationOutstandingSAXIS(addr *common.Address, toStaker *hexutil.Big) (*hexutil.Big, error) {
	val, err := p.rpc.DelegationOutstandingSAXIS(addr, toStaker.ToInt())
	if err != nil {
		return nil, err
	}
	return (*hexutil.Big)(val), nil
